...
# Prefer clang/lld over gcc collect2 — gcc-14 collect2 intermittently SEGFAULTS linking
# proc-macro .so crates under QEMU (cross-arch docker on Apple Silicon).
...
cp "$SRC/$FUZZ_DIR/target/$TRIPLE/release/gltf_parse" /mayhem/gltf-parse
echo "built /mayhem/gltf-parse"

echo "=== cargo test --no-run -p gltf-json --features names (clean flags, for test.sh) ==="
( cd "$SRC" && env -u RUSTFLAGS CARGO_TARGET_DIR="$SRC/mayhem/test-target" \
  cargo test --no-run -p gltf-json --features names )
